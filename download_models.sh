...
if [ ! -f "$CHECKPOINT_DIR/sd_xl_base_1.0.safetensors" ]; then
    ...
    curl -sfLS -C - "https://huggingface.co/stabilityai/stable-diffusion-xl-base-1.0/resolve/main/sd_xl_base_1.0.safetensors" \
...
if [ ! -f "$LORA_DIR/sdxl_lightning_8step_lora.safetensors" ]; then
    ...
    curl -sfLS -C - "https://huggingface.co/ByteDance/SDXL-Lightning/resolve/main/sdxl_lightning_8step_lora.safetensors" \
...
if [ ! -f "$UPSCALE_DIR/4x-UltraSharp.pth" ]; then
    ...
    curl -sfLS -C - "https://huggingface.co/lokCX/4x-Ultrasharp/resolve/main/4x-UltraSharp.pth" \
...
if [ ! -f "$CONTROLNET_DIR/controlnet-tile-sdxl.safetensors" ]; then
    ...
    curl -sfLS -C - "https://huggingface.co/xinsir/controlnet-tile-sdxl-1.0/resolve/main/diffusion_pytorch_model.safetensors" \
         -o "$CONTROLNET_DIR/controlnet-tile-sdxl.safetensors"
fi

echo "✓ All required models verified successfully!"

# If arguments were passed to the script, execute them
if [ $# -gt 0 ]; then
    exec "$@"
# Otherwise, automatically launch the batch processing pipeline
elif [ -f "/app/run_usdu_batch.py" ] && [ "$MODE" != "endpoint" ]; then
    echo "=== Launching Batch Upscale Job ==="
